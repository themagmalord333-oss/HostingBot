...
try:
    # 5 Second timeout added. Hang nahi hoga.
    mongo_client = MongoClient(config.MONGO_URI, serverSelectionTimeoutMS=5000)
    mongo_client.admin.command('ping') # Force connection test
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if data == "owner_auto_on": 
        ...
        else: projects_col.update_one({"_id": proj["_id"]}, {"$set": {"action": "restart"}}); await query.answer("🔄 Restart signal sent...")
...
async def main_node():
    ...
    try:
        ...
        print(f"❌ Telegram Error (Check Token or FloodWait): {e}")
        exit(1)
